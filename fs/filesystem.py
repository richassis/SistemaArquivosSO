...
i-nodes, leitura/escrita de conteudo, diretorios, links simbolicos,
permissoes e resolucao de caminhos absolutos/relativos.
"""
...
class FileSystem:
    ...
    def remove_dir_entry(self, dir_inode_num: int, name: str, check_perm: bool = True) -> None:
        if check_perm:
            dir_inode = self.read_inode(dir_inode_num)
            self._check_perm(dir_inode, "w")
        for b in self._iter_blocks(dir_inode_num):
    ...
    # Permissoes
    # ------------------------------------------------------------------
    def _check_perm(self, inode: Inode, mode: str) -> None:
        need = {"r": PERM_R, "w": PERM_W, "x": PERM_X}[mode]
        bits = (inode.perm >> 3) & 0b111 if inode.owner == self.current_user else inode.perm & 0b111
        if not (bits & need):
            raise FSError(f"permissão negada ({mode}) para '{inode.name}'")
    ...
    def mv(self, src: str, dst: str) -> None:
        ...
        if src_inode.type == TYPE_DIR and dst_parent != src_parent:
            self.remove_dir_entry(src_num, "..", check_perm=False)
