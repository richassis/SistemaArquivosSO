...
Mais alguns comandos de apoio (pwd, whoami, su, stat, df, help, reset)
uteis para demonstrar persistencia, permissoes e uso do disco.
"""
...
class Shell:
    ...
    def _dispatch(self, tokens) -> bool:
        ...
        if cmd in ("exit", "quit"):
            ...
            print(fs.current_user)

        elif cmd == "su":
            if not args:
                raise FSError("uso: su <usuario>")
            fs.current_user = args[0]
            print(f"usuário atual: {fs.current_user}")
  ...
  Outros:
    whoami / su <usuario>          usuário atual / troca de usuário (permissões)
